...
class Safe {
  ...
  async signTransaction(
    safeTransaction: SafeTransaction | any,
  ...
  /**
   * Converts a transaction from type any to type SafeTransaction
   *
  ...
  async toSafeTransactionType(
    serviceTransactionResponse: any
  ): Promise<SafeTransaction> {
    const safeTransactionData: SafeTransactionDataPartial = {
    ...
    const safeTransaction = await this.createTransaction({ safeTransactionData })
    serviceTransactionResponse.confirmations?.map((confirmation: any) => {
